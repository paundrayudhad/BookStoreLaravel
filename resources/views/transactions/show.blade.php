...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="p-6 flex items-start space-x-4">
        ...
        <!-- Book Details and Download Button -->
        <div class="flex-1 min-w-0 flex flex-col justify-between">
            <div>
                ...
                <div class="mt-2 flex items-center justify-between">
                    ...
                    <span class="text-lg font-bold text-orange-600">Rp {{ number_format($detail->price * $detail->quantity, 0, ',', '.') }}</span>
                </div>
            </div>

            @if($transaction->status === 'completed' && $detail->book->pdf_file)
                ...
                <div class="space-y-3">
                    @if($transaction->status === 'pending')
                        <a href="{{ route('payments.create', $transaction) }}"
                           class="block w-full bg-orange-600 hover:bg-orange-700 text-white py-2 px-4 rounded-lg font-medium text-center transition duration-150 ease-in-out">
                            Bayar Sekarang
                        </a>
                    @endif
